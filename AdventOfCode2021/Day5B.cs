...
namespace AdventOfCode2021
{
    internal class Day5B : IPuzzle
    {
        ...
        public string GetSolution()
        {
            ...
            {
                if (line.Origin.X == line.Terminus.X) // Vertical
                {
                    var init = Math.Min(line.Origin.Y, line.Terminus.Y);
                    var limit = Math.Max(line.Origin.Y, line.Terminus.Y);
                    for (int y = init; y <= limit; y++)
                    {
                        AddPoint(line.Origin.X, y);
                    }
                }
                else if (line.Origin.Y == line.Terminus.Y) // Horizontal
                {
                    var init = Math.Min(line.Origin.X, line.Terminus.X);
                    var limit = Math.Max(line.Origin.X, line.Terminus.X);
                    for (int x = init; x <= limit; x++)
                    {
                        AddPoint(x,line.Origin.Y);
                    }
                }
                else
                {
                    Console.WriteLine($"Origin: {line.Origin.X},{line.Origin.Y} Terminus: {line.Terminus.X}{line.Terminus.Y}");
                    float xDiff = line.Terminus.X - line.Origin.X;
                    float yDiff = line.Terminus.Y - line.Origin.Y;
                    ...
                    while (x != line.Terminus.X + xMod && y != line.Terminus.Y +yMod)
                    {
            ...
            var cnt = points.Count(x => x.Value > 1);
            return cnt.ToString();
        }
        ...
        private IList<Line> GetLines()
        {
            var lines = Input.Split('|').ToList()
                .Select(x => GetLine(x)).ToList();
            return lines;
        }
        ...
        private Line GetLine(string s)
        {
            ...
            var values = regex.Matches(s).ToList().Select(x => Int32.Parse(x.Value)).ToArray();
            return new Line
            {
                Origin = new Point(values[0], values[1]),
                Terminus = new Point(values[2], values[3])
            };
        }

        private Orientation GetOrientation(Line line, Point point)
        {
            int val = (line.Terminus.Y - line.Origin.Y) * (point.X - line.Terminus.X)
                - (line.Terminus.X - line.Origin.X) * (point.Y - line.Terminus.Y);
